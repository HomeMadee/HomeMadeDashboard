...
class CategoryAPIController extends Controller
{
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getCategoryCuisine($id)
    {

        /** @var Category $category */
        if (!empty($this->categoryRepository)) {
            $category = $this->categoryRepository->findByField('cuisine_id',($id));
        }

        if (empty($category)) {
            return $this->sendError('Category not found soory');
        }

        return $this->sendResponse($category->toArray(), 'Category Cuisine retrieved successfully');
    }
