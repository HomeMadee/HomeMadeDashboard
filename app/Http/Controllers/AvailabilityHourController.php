...
class AvailabilityHourController extends Controller
{
    ...
    public function __construct(AvailabilityHourRepository $availabilityHourRepo, CustomFieldRepository $customFieldRepo, RestaurantRepository $restaurantRepo)
    {
        parent::__construct();
    ...
    {
        $resturant = $this->restaurantRepository->getByCriteria(new RestaurantsOfUserCriteria(auth()->id()))->pluck('name', 'id');
        ...
        return view('availability_hours.create')->with("customFields", isset($html) ? $html : false)->with("restaurant", $resturant);
    }
    ...
    public function edit(int $id)
    {
        //$this->availabilityHourRepository->pushCriteria(new AvailabilityHoursOfUserCriteria(auth()->id()));
        $availabilityHour = $this->availabilityHourRepository->findWithoutFail($id);
        $resturant = $this->restaurantRepository->getByCriteria(new ResturantsOfUserCriteria(auth()->id()))->pluck('name', 'id');
        ...
        return view('availability_hours.edit')->with('availabilityHour', $availabilityHour)->with("customFields", isset($html) ? $html : false)->with("resturant", $resturant);
    }
