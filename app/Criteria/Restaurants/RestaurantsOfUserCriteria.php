...
class RestaurantsOfUserCriteria implements CriteriaInterface
{
    ...
    public function apply($model, RepositoryInterface $repository)
    {
        if (auth()->user()->hasRole('admin')) {
            ...
            return $model->join('user_restaurants', 'user_restaurants.restaurant_id', '=', 'restaurant.id')
                ->select('restaurants.*')
                ->where('user_restaurants.user_id', $this->userId);
        } elseif (auth()->user()->hasRole('driver')) {
            return $model->join('driver_restaurants', 'driver_restaurants.restaurant_id', '=', 'restaurant.id')
                ->select('restaurants.*')
